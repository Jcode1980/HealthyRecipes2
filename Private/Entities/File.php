<?php 
	...
	{ 
		...
		public function filePath(){
			...
			$filePath = FILES_PRODUCTION_FOLDER . $this->className() . "/" . $this->vars["id"];
		...
		public function thumbnailImagePath(){
			$filePath = FILE_PREVIEWS_PRODUCTION_FOLDER . $this->className() . "/". $this->vars["id"];
			debugln("FILE_PREVIEWS_PRODUCTION_FOLDER: " . FILE_PREVIEWS_PRODUCTION_FOLDER);
		...
		public static function objectForID($encryptedID){
			...
			if ($id) {
				return BLGenericRecord::recordMatchingKeyAndValue("File", "id", $id);
			}
			else{
				return null;
			}
